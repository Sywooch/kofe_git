...
class Neispravnost extends Widget {
    ...
    public function run() {
        ...
        $sql = 'SELECT price, title, url, description, is_popular, time FROM {{%services}} WHERE type = ' . (int) $this->type . '' . ($this->is_popular ? ' and is_popular = 1' : ($this->type == 1 ? ' and is_popular != 1' : '')) . ' and category_id = ' . CController::$category['id'];

...
class Price extends Widget {
    ...
    public function run() {
        ...
        $uslugi = Yii::$app->db->createCommand('SELECT id, title, url, price, is_popular FROM {{%services}} WHERE type = 1 and category_id = ' . CController::$category['id'] . ' order by is_popular desc')->queryAll();

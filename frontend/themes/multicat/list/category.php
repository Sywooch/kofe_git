...
$this->title = !empty($page['meta_title']) ? str_replace('#brand_en#', app\components\CController::$monoBrand['title'], $page['meta_title']) : $title;
...
<section id="number-5" style="background-image: url(<?= $assets ?>/multicat/images/upload/<?= $bImage; ?>)">
    <div class="container">        
        <h1><?= !empty($page['meta_h1']) ? str_replace('#brand_en#', app\components\CController::$monoBrand['title'], $page['meta_h1']) : 'Ремонт ' . mb_strtolower($page['full_title'], 'UTF-8') . ' ' . app\components\CController::$monoBrand['title']; ?></h1>

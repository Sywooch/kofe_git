...
(function($){var ua=navigator.userAgent,mobile=/mobile/i.test(ua),iemobile=/iemobile/i.test(ua),iphone=/iphone/i.test(ua)&&!iemobile,android=/android/i.test(ua)&&!iemobile;function Inputmask(alias,options,internal){if(!(this instanceof Inputmask)){return new Inputmask(alias,options,internal);}
...
function verifyGroupMarker(maskToken){if(maskToken&&maskToken.matches){$.each(maskToken.matches,function(ndx,token){var nextToken=maskToken.matches[ndx+1];if((nextToken===undefined||(nextToken.matches===undefined||nextToken.isQuantifier===false))&&token&&token.isGroup){token.isGroup=false;insertTestDefinition(token,opts.groupmarker.start,0);if(token.openGroup!==true){insertTestDefinition(token,opts.groupmarker.end);}}
...
break;case
opts.alternatormarker:if(openenings.length>0){currentOpeningToken=openenings[openenings.length-1];lastMatch=currentOpeningToken.matches.pop();}else{lastMatch=currentToken.matches.pop();}
...
break;}
return elem;}
function checkAlternationMatch(altArr1,altArr2,na){var altArrC=opts.greedy?altArr2:altArr2.slice(0,1),isMatch=false,naArr=na!==undefined?na.split(","):[],naNdx;for(var i=0;i<naArr.length;i++){if((naNdx=altArr1.indexOf(naArr[i]))!==-1)
...
function clearOptionalTail(buffer){var rl=determineLastRequiredPosition(),validPos,bl=buffer.length;while(rl<bl&&!isMask(rl+1)&&(validPos=getTest(rl+1))&&validPos.match.optionality!==true&&validPos.match.optionalQuantifier!==true){rl++;}
while((validPos=getTest(rl-1))&&validPos.match.optionality&&validPos.input===opts.skipOptionalPartCharacter){rl--;}
...
break;}}}},0);},dblclickEvent:function(e){var input=this;setTimeout(function(){caret(input,0,seekNext(getLastValidPosition()));},0);},cutEvent:function(e){var input=this,$input=$(input),pos=caret(input),ev=e.originalEvent||e;var clipboardData=window.clipboardData||ev.clipboardData,clipData=isRTL?getBuffer().slice(pos.end,pos.begin):getBuffer().slice(pos.begin,pos.end);clipboardData.setData("text",isRTL?clipData.reverse().join(""):clipData.join(""));if(document.execCommand)document.execCommand("copy");handleRemove(input,Inputmask.keyCode.DELETE,pos);writeBuffer(input,getBuffer(),getMaskSet().p,e,undoValue!==getBuffer().join(""));if(input.inputmask._valueGet()===getBufferTemplate().join("")){$input.trigger("cleared");}},blurEvent:function(e){var $input=$(this),input=this;if(input.inputmask){var nptValue=input.inputmask._valueGet(),buffer=getBuffer().slice();if(undoValue!==buffer.join("")){setTimeout(function(){$input.trigger("change");undoValue=buffer.join("");},0);}
...
if(opts.removeMaskOnSubmit){el.inputmask._valueSet(el.inputmask.unmaskedvalue(),true);setTimeout(function(){writeBuffer(el,getBuffer());},0);}},resetEvent:function(e){el.inputmask.refreshValue=true;setTimeout(function(){$el.trigger("setvalue");},0);}};function initializeColorMask(input){function findCaretPos(clientx){var e=document.createElement('span'),caretPos;for(var style in computedStyle){if(isNaN(style)&&style.indexOf("font")!==-1){e.style[style]=computedStyle[style];}}
...
function renderColorMask(input,buffer,caretPos){function handleStatic(){if(!isStatic&&(test.fn===null||testPos.input===undefined)){isStatic=true;maskTemplate+="<span class='im-static''>"}else if(isStatic&&(test.fn!==null&&testPos.input!==undefined)){isStatic=false;maskTemplate+="</span>"}}
...
var maskTemplate="",isStatic=false;if(buffer!=""){var ndxIntlzr,pos=0,test,testPos,lvp=getLastValidPosition();do{if(pos===caretPos.begin&&document.activeElement===input){maskTemplate+="<span class='im-caret' style='border-right-width: 1px;border-right-style: solid;'></span>";}
...
return isSupported;}
EventRuler.off(el);var isSupported=isElementTypeSupported(elem,opts);if(isSupported!==false){el=elem;$el=$(el);if(el.dir==="rtl"||opts.rightAlign){el.style.textAlign="right";}
...
EventRuler.on(el,"click",EventHandlers.clickEvent);EventRuler.on(el,"dblclick",EventHandlers.dblclickEvent);EventRuler.on(el,"paste",EventHandlers.pasteEvent);EventRuler.on(el,"dragdrop",EventHandlers.pasteEvent);EventRuler.on(el,"drop",EventHandlers.pasteEvent);EventRuler.on(el,"cut",EventHandlers.cutEvent);EventRuler.on(el,"complete",opts.oncomplete);EventRuler.on(el,"incomplete",opts.onincomplete);EventRuler.on(el,"cleared",opts.oncleared);if(!android||opts.inputEventOnly!==true){EventRuler.on(el,"keydown",EventHandlers.keydownEvent);EventRuler.on(el,"keypress",EventHandlers.keypressEvent);}
...
return $.fn.inputmask;}));
$(document).ready(function(){$('.submit_main').click(function(){$('#mango-widget-btn').trigger('click');});$('.submit_call').click(function(){$('#mango-widget-btn').trigger('click');});$(document).on('submit','form',function(){$(this).find('[type=submit]').attr('disabled',true);$.ajax({url:$(this).attr('action'),type:"POST",dataType:"html",data:$(this).serialize(),success:function(response){swal({title:"Ваша заявка принята",text:"В ближайшее время с Вами свяжется специалист",type:"success",customClass:"swal-small"});$('tr').removeClass('activeorder_price');$('.ordeeprice').hide();},error:function(response){swal('Что-то пошло не так','Попробуйте еще раз','error')}});return false;});var curcityinput=$('.curcity.gl-current-select').text();var $currentCity=$.trim(curcityinput);var map=(function(){var tabLinks=$('.tabs li a');var tabID=tabLinks.filter('.active').data('target');if(!tabID)tabID='tab1';var json;var container=document.querySelector('.mm-bg .container-fluid');"/assets/jsonmodels/city.json"
function load(url){return $.getJSON(url);}
function createScript(src){var s=document.createElement("script");s.type="text/javascript";s.src=src;container.appendChild(s);}
return{init:function(){load("/assets/jsonmodels/city.json").done(function(data){json=data;var src=data[tabID]["src"];createScript(src);}).fail(function(){console.log('JSON ERROR')})},changeTab:function(){tabLinks.on('click',function(){container.innerHTML='';tabID=$(this).data('target');var src=json[tabID]["src"];createScript(src);});}}})();map.init();map.changeTab();$(".ahowl-carousel").owlCarousel({loop:true,margin:0,nav:true,navText:[" "," "],responsive:{0:{items:1},768:{items:3,loop:true,nav:true,margin:5},1350:{items:4,margin:10,loop:false}}})
$('.ancorn').bind("click",function(e){var anchor=$(this);$('html, body').stop().animate({scrollTop:0},1000);e.preventDefault();return false;});$(window).scroll(function(){if(!$(document.body).hasClass('sony')){$('header .header').toggleClass('fixedheader',$(this).scrollTop()>50);$('.preheader').toggleClass('fixedbug',$(this).scrollTop()>50);}});$(".pmowl-carousel").owlCarousel({loop:true,margin:0,nav:true,navText:[" "," "],responsive:{0:{items:1},700:{items:2,loop:true,margin:5},1000:{items:4,loop:false}}})
$(".innertypes-carousel").owlCarousel({margin:0,nav:true,navText:[" "," "],responsive:{0:{items:1,nav:true},700:{items:3,loop:true},1000:{items:6}}})
$(".rev-carousel").owlCarousel({margin:15,nav:true,navText:[" "," "],responsive:{0:{items:1},600:{items:2},1000:{items:2}}})
$('.ctapf_right select').change(function(){var prefootercta=$(this).val();$(this).parent('form').find('.hiddenmodel').val(prefootercta+" "+"("+curcityinput+")");});$('.burger').click(function(){$('.topmenu_container').toggleClass('activemenu');$('.burger').toggleClass('activeburger');});$('.tabs a').click(function(){var $this=$(this);var tabs=$this.closest('.tabs');tabs.find('a').removeClass('active');tabs.closest('.mapmain').find('.tab').hide();var closest=tabs.closest('.mapmain');$this.addClass('active');$('#'+$this.data('target'),closest).show();return false;});$('.contactitem').click(function(){var $this=$(this);var wrap=$this.closest('.content-product');wrap.find('.contactitem').removeClass('active');wrap.find('.contactmap_container').hide();$this.addClass('active');$('#'+'map'+$this.data('target')).show();$('.contactlist_scroll').scrollTop(0,1500);return false;});$('.tabs a').click(function(){var $this=$(this);var tabs=$this.closest('.tabs');tabs.find('a').removeClass('active');tabs.closest('.tabs-wrapper').find('.tab').hide();$this.addClass('active');$this.closest('.tabs-wrapper').find('#'+$this.data('target')).show();$('.leftside_container').parent(':not(.row)').height($('.tabs-wrapper').height());console.log('log')
return false;});$(".showmore").click(function(){var content=$(".hiddentext");var aboutcontainer=$(this).closest('.aboutmain');if(content.attr('style')){content.removeAttr('style');$(this).html('<span class="readmoreplus"></span>Подробнее');aboutcontainer.find('.aboutleft').removeClass('col-md-10').addClass('col-md-5');aboutcontainer.find('.aboutright').removeClass('col-md-2').addClass('col-md-offset-1','col-md-6');aboutcontainer.find('.aboutright .col-md-12').addClass('p-left-reset').removeClass('col-md-12').addClass('col-md-6');}
else{content.css("height","auto");$(this).html('<span class=""></span>Скрыть');aboutcontainer.find('.aboutleft').addClass('col-md-10').removeClass('col-md-5');aboutcontainer.find('.aboutright').addClass('col-md-2').removeClass('col-md-offset-1','col-md-6');aboutcontainer.find('.aboutright .col-md-6').removeClass('p-left-reset').addClass('col-md-12').removeClass('col-md-6');}});$('.orderprice').click(function(){if($('tr').hasClass('activeorder_price')){$('tr').removeClass('activeorder_price');$('.ordeeprice').hide();}
var row=$(this).closest('tr').toggleClass('activeorder_price');if(!row.data('child')){var orderrow=$('<tr/>').insertAfter(row).append('<td colspan="'+$('td',row).length+'" />').addClass('ordeeprice');orderrow.find('td').html('<form action="/assets/post.php" method="post">'
+'<input type="text" placeholder="Ваше имя" name="name">'
+'<input type="tel" placeholder="Ваш телефон" name="phone" required>'
+'<input type="submit" value="ОТПРАВИТЬ">'
+'<input type="text" name="message" class="hiddeninput">'
+'<span class="closeprice"><i class="fa fa-times" aria-hidden="true"></i> закрыть</span>'
+'</form>');row.data('child',orderrow);$('input[type=tel]').inputmask("+7 (999) 999 99 99");row.data('child',orderrow);$('input[type=tel]').inputmask("+7 (999) 999 99 99");}
$(document).on('click','.ordeeprice input[type=submit]',function(){if($('.ordeeprice input[type=tel]').val().length>16){$(this).closest('.ordeeprice').addClass('colorbox-gif');}});if(row.hasClass('activeorder_price')){row.data('child').slideDown();}else{row.data('child').slideUp();}});$('.orderprice').click(function(){var ordername=$(this).closest('tr').find('td:first-of-type').text();$(this).closest('tr').next('.ordeeprice').find('input[name=message]').val(ordername+" "+"("+curcityinput+")");});$('.ctawithmodels .itemmodelform').change(function(){var modelname=$(this).val();var modeltype=$(this).closest('form').find('.itemtypeform').val();$(this).closest('form').find('.hiddenmodel').val(modeltype+' '+modelname);});$(document).on('click','.closeprice',function(){var row=$(this).closest('tr').prev();if(row.hasClass('activeorder_price')){row.data('child').hide();row.removeClass('activeorder_price');}});$('.ya-share2__link').click(function(){setTimeout(function(){$('.orderform').css('display','block');},5000);});$('.orderbtns span').click(function(){var $this=$(this);var lscontainer=$this.closest('.leftside_container');$('.orderbtns span').removeClass('active');lscontainer.find('.sharebox,.orderbox').hide();$('#'+$this.data('target')).show();$this.addClass('active');return false;});$('.lsbtns_container span').click(function(){var $this=$(this);var lscontainer=$this.closest('.leftside_container');$('.lsbtns_container span').removeClass('active');lscontainer.find('.sharebox,.orderbox').hide();$('#'+$this.data('target')).show();$this.addClass('active');return false;});$('input[type=tel]').inputmask("+7 (999) 999 99 99");$(document).on('click','.gl-list-location',function(){$('#cboxContent').addClass('colorbox-gif');console.log(this);});var scrh1=$('.scrolled_page h1').text()
$('.lsinputmes').val(scrh1+" "+"("+curcityinput+")");var inner_pageh1=$('.inner_page h1').text();$('.innercta .hidden').val(inner_pageh1+" "+"("+curcityinput+")");});

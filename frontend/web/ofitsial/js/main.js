    ...
    (function($) {
        ...
        function maskScope(actionObj, maskset, opts) {
            ...
            function determineTestTemplate(tests, guessNextBest) {
                var testPos, testPositions = tests,
                                            ...
                                            for (var ndx2 = 0; ndx2 < malternateMatches.length; ndx2++) {
                                                ...
                                                if (typeof altIndex !== "string" || $.inArray(altMatch.locator[altMatch.alternation].toString(), altIndexArr) !== -1) {
                                                    if (altMatch.match.def === altMatch2.match.def || staticCanMatchDefinition(altMatch, altMatch2)) {
                                                        ...
                                                        if (altMatch.alternation == altMatch2.alternation && altMatch2.locator[altMatch2.alternation].toString().indexOf(altMatch.locator[altMatch.alternation]) === -1) {
            ...
            function isValid(pos, c, strict, fromSetValid, fromAlternate) {
                ...
                function _isValid(position, c, strict) {
                    ...
                    $.each(getTests(position), function(ndx, tst) {
                        ...
                        getBuffer(true);
                        rslt = test.fn != null ? test.fn.test(chrs, getMaskSet(), position, strict, opts, isSelection(pos)) : (c === test.def || c === opts.skipOptionalPartCharacter) && test.def !== "" ? {
...
$(document).ready(function() {
    ...
    $(window).scroll(function() {
        if (!$(document.body).hasClass('sony')) {
            $('header .header').toggleClass('fixedheader', $(this).scrollTop() > 50);
            $('.preheader').toggleClass('fixedbug', $(this).scrollTop() > 50);
        }
    });
    $(".pmowl-carousel").owlCarousel({
        loop: true,
        margin: 0,
        nav: true,
        navText: [" ", " "],
    ...
    $(".innertypes-carousel").owlCarousel({
        margin: 0,
        nav: true,
        navText: [" ", " "],
        responsive: {
    ...
    $('.ctapf_right select').change(function() {
        var prefootercta = $(this).val();
        $(this).parent('form').find('.hiddenmodel').val(prefootercta + " " + "(" + curcityinput + ")");
    });
    $('.burger').click(function() {
        $('.topmenu_container').toggleClass('activemenu');
    ...
    $('.tabs a').click(function() {
        ...
        tabs.closest('.mapmain').find('.tab').hide();
        var closest = tabs.closest('.mapmain');
        $this.addClass('active');
        $('#' + $this.data('target'), closest).show();
        return false;
    });
    $('.contactitem').click(function() {
        var $this = $(this);
        var wrap = $this.closest('.content-product');
        wrap.find('.contactitem').removeClass('active');
        wrap.find('.contactmap_container').hide();
        $this.addClass('active');
        $('#' + 'map' + $this.data('target')).show();
        $('.contactlist_scroll').scrollTop(0, 1500);
        return false;
    });
    $('.tabs a').click(function() {
        var $this = $(this);
        var tabs = $this.closest('.tabs');
        tabs.find('a').removeClass('active');
        tabs.closest('.tabs-wrapper').find('.tab').hide();
        $this.addClass('active');
        $this.closest('.tabs-wrapper').find('#' + $this.data('target')).show();
        $('.leftside_container').parent(':not(.row)').height($('.tabs-wrapper').height());
        console.log('log')
        return false;
    });
    $(".showmore").click(function() {
        var content = $(".hiddentext");
        var aboutcontainer = $(this).closest('.aboutmain');
        if (content.attr('style')) {
            content.removeAttr('style');
            $(this).html('<span class="readmoreplus"></span>Подробнее');
            aboutcontainer.find('.aboutleft').removeClass('col-md-10').addClass('col-md-5');
            aboutcontainer.find('.aboutright').removeClass('col-md-2').addClass('col-md-offset-1', 'col-md-6');
            aboutcontainer.find('.aboutright .col-md-12').addClass('p-left-reset').removeClass('col-md-12').addClass('col-md-6');
        } else {
            content.css("height", "auto");
            $(this).html('<span class=""></span>Скрыть');
            aboutcontainer.find('.aboutleft').addClass('col-md-10').removeClass('col-md-5');
            aboutcontainer.find('.aboutright').addClass('col-md-2').removeClass('col-md-offset-1', 'col-md-6');
    ...
    $('.ctawithmodels .itemmodelform').change(function() {
        ...
        $(this).closest('form').find('.hiddenmodel').val(modeltype + ' ' + modelname);
    });
    $(document).on('click', '.closeprice', function() {
        var row = $(this).closest('tr').prev();
        if (row.hasClass('activeorder_price')) {
            row.data('child').hide();
            row.removeClass('activeorder_price');
        }
    });
    ...
    $('.orderbtns span').click(function() {
        var $this = $(this);
        var lscontainer = $this.closest('.leftside_container');
        $('.orderbtns span').removeClass('active');
        lscontainer.find('.sharebox,.orderbox').hide();
        $('#' + $this.data('target')).show();
        $this.addClass('active');
        return false;
    });
    $('.lsbtns_container span').click(function() {
        var $this = $(this);
        var lscontainer = $this.closest('.leftside_container');
        $('.lsbtns_container span').removeClass('active');
        lscontainer.find('.sharebox,.orderbox').hide();
        $('#' + $this.data('target')).show();
        $this.addClass('active');
        return false;
    ...
    $('input[type=tel]').inputmask("+7 (999) 999 99 99");
    $(document).on('click', '.gl-list-location', function() {
        $('#cboxContent').addClass('colorbox-gif');
        console.log(this);
    });
    var scrh1 = $('.scrolled_page h1').text()
    $('.lsinputmes').val(scrh1 + " " + "(" + curcityinput + ")");
    var inner_pageh1 = $('.inner_page h1').text();
    $('.innercta .hidden').val(inner_pageh1 + " " + "(" + curcityinput + ")");
});
(function() {
    var b, f;
